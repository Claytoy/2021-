...
namespace GeodesyLib
{
    ...
    {
        public static DataEntity Read(string dataFile)
        {
            DataEntity obs = new DataEntity();
            Ellipsoid datum = new Ellipsoid();
            var data = ReadNegData(dataFile, ref datum);
            obs.Datum = datum;
            obs.Data = data;
            AfterRead(ref obs);
            return obs;
        ...
        static void AfterRead(ref DataEntity obs)
        {
        ...
        static List<GeodesicInfo> ReadNegData(string filepath, ref Ellipsoid ellipsoid)
        {
        ...
        static List<GeodesicInfo> ReadPosData(string filepath, ref Ellipsoid ellipsoid)
        {
            List<GeodesicInfo> dataInfo = new List<GeodesicInfo>();
            try
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    p1.B = double.Parse(info[1]);
                    p1.L = double.Parse(info[2]);
                    date.A12 = double.Parse(info[3]);
                    date.S = double.Parse(info[4]);
        ...
        public static StringBuilder GetReport(Ellipsoid ellipsoid, List<GeodesicInfo> data, int type)
        {
            ...
            line = "------------------------------------------------------------------------计算结果---------------------------------------------------------------------------\r\n\r\n";
            sb.Append(line);
            line = "\t点名\t纬度（B）\t\t精度（L）\t\t大地方位角（A）\t\t大地线（S）\r\n\r\n";
            sb.Append(line);
            for (int i = 0; i < data.Count; i++)
            {
                ...
                line = "-------------------------------------------------------------------------------------------------------------------------\r\n\r\n";
                sb.Append(line);
                line = "\t" + p1.Name.PadRight(20) + GeoPro.DMS2String(p1.B).PadRight(20) + GeoPro.DMS2String(p1.L).PadRight(20) + GeoPro.DMS2String(info.A12).PadRight(20) + info.S.ToString("0.000") + "\t\t\r\n\r\n";
                sb.Append(line);
                line = "\t" + p2.Name.PadRight(20) + GeoPro.DMS2String(p2.B).PadRight(20) + GeoPro.DMS2String(p2.L).PadRight(20) + GeoPro.DMS2String(info.A21).PadRight(20) + info.S.ToString("0.000") + "\t\t\r\n\r\n";
                sb.Append(line);
                line = "-------------------------------------------------------------------------------------------------------------------------\r\n\r\n";
                sb.Append(line);
            }
