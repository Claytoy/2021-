...
namespace GeodesyLib
{
    ...
    public class GeoPro
    {
        ...
        public static double RAD2DMS(double rad)
        {
            ...
            {
                sign = -1;
                rad = System.Math.Abs(rad);
            }
            sec = rad * 180.0 / Math.PI * 3600.0;
            deg = Convert.ToInt32(Math.Floor(sec / 3600.0));
            min = Convert.ToInt32(Math.Floor((sec - deg * 3600.0) / 60.0));
            sec = sec - deg * 3600.0 - min * 60.0;
            if (sec < 0)
                sec = 0;
        ...
        {
            double alpha = 0;
            alpha = (e1 * e1 / 2 + Math.Pow(e1, 4) / 8 + Math.Pow(e1, 6) / 16)
                - (Math.Pow(e1, 4) / 16 + Math.Pow(e1, 6) / 16) * cos2_A0
                + (3 * Math.Pow(e1, 6) / 128) * cos2_A0 * cos2_A0;
            return alpha;
        }
        /// <summary>
        /// 获取Beta参数
        ...
        public static string DMS2String(double arc)
        {
            ...
            str = (sign * dd).ToString() + "°" + mm.ToString("00") + "′" + ss.ToString("00.0000") + "″";
            return str;
