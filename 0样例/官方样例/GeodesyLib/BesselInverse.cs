...
namespace GeodesyLib
{
    ...
    public class BesselInverse
    {
        ...
        private void CalA1_Lamda(double dL, double u2, double u1, double[] ab, ref double lamda, ref double A1,
            ref double sigma, ref double cos2_A0)
        {
            double deltat = 0, delta = 0;
            double cosSigma = 0, sinSigma = 0;
            double e1 = Ell.e1;
            ...
            do
            {
                ...
                sinSigma = p * Math.Sin(A1) + q * Math.Cos(A1);
                cosSigma = ab[0] + ab[1] * Math.Cos(lamda);
                sigma = Math.Atan(sinSigma / cosSigma);
                sigma = GeoPro.InvJudgedel(sigma, cosSigma);
                ...
                delta = (alpha * sigma + (beta) * Math.Cos(2 * sigma1 + sigma) * Math.Sin(sigma)
        ...
        public void InverseSolution(GeodesicInfo geodesic)
        {
            ...
            else
            {
                ...
                CalA1_Lamda(dL, u2, u1, ab, ref lamda, ref A12, ref sigma, ref cos2_A0);
                ...
                double xs12 = ABC[2] * Math.Sin(2 * sigma) * Math.Cos(4 * sigma1 + 2 * sigma);

                S = (sigma - ABC[1] * Math.Sin(sigma) * Math.Cos(2 * sigma1 + sigma) - xs12) / ABC[0];
                //计算A2
                ...
                if (A21 < 0) A21 = A21 + Math.PI * 2;
                  else if(A21>Math.PI*2)A21=A21- Math.PI * 2;
                //
                if (A12 >= Math.PI&&A21>=Math.PI) A21 = A21 - Math.PI;
                if (A12 < Math.PI&&A12<Math.PI) A21 = A21 + Math.PI;
            }
