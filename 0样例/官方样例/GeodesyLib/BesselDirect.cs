...
namespace GeodesyLib
{
    ...
    public class BesselDirect
    {
             ...
             ref double sigma)
        {
            ...
            {
                temp = sigma;
                sigma = ABC[0] * S + ABC[1] * Math.Sin(temp) * Math.Cos(2 * delta1 + temp);
                sigma = sigma + ABC[2] * Math.Sin(2 * temp) * Math.Cos(4 * delta1 + 2 * temp);

            } while (Math.Abs(temp - sigma) > 0.000000001);
        }
        ...
        public void DirectSolution(GeodesicInfo geodesic)
        {
            ...
            double cotSigma1 = cosu1 * Math.Cos(A12) / sinu1;
            double sigma1 = Math.Atan(1.0 / cotSigma1);
            ...
            //计算终点大地坐标及大地方位角
            double sinu2 = sinu1 * Math.Cos(sigma) + cosu1 * Math.Cos(A12) * Math.Sin(sigma);
            double B2 = Math.Atan(1 / Math.Sqrt(1 - e1 * e1) * sinu2 / Math.Sqrt(1 - sinu2 * sinu2));

            double lamba = Math.Atan(Math.Sin(sigma) * Math.Sin(A12) /
                (cosu1 * Math.Cos(sigma) - sinu1 * Math.Sin(sigma) * Math.Cos(A12)));
            lamba = GeoPro.DirJudgelamba(Math.Sin(A12), lamba);

            double L2 = L1 + lamba - lamda_L;

            double A21 = Math.Atan(cosu1 * Math.Sin(A12) /
                (cosu1 * Math.Cos(sigma) * Math.Cos(A12) - sinu1 * Math.Sin(sigma)));
            ...
            var P2 = new PointInfo();
            P2.Name = geodesic.P2.Name;
            P2.B = GeoPro.RAD2DMS(B2); ;
            P2.L = GeoPro.RAD2DMS(L2);
            geodesic.P2 = P2;
